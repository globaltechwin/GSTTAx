...
export function InvoiceClient() {
  ...
  const [dialog, setDialog] = useState<{ type: "delete"; invoice: InvoiceData } | null>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/invoices/${dialog.invoice.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        setDialog(null);
  ...
  const columns: ColumnDef<InvoiceData>[] = [
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        const displayNo = row.invoiceNo || row.orderId;
        return <span className="text-foreground font-mono text-xs font-semibold">{displayNo}</span>;
      },
      size: 120,
    },
    {
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return (
          <span className={cn("rounded-md px-2 py-0.5 text-xs font-medium", color)}>{label}</span>
        );
      },
      size: 100,
    },
    {
      ...
      cell: (info) => {
        ...
        return <span className="text-foreground text-sm font-medium">{formatCurrency(val)}</span>;
      },
      size: 120,
    },
    {
      ...
      cell: (info) => {
        const val = info.getValue() as number | null;
        return <span className="text-foreground text-sm font-semibold">{formatCurrency(val)}</span>;
      },
      size: 120,
    },
    {
        ...
        return (
          <div className="relative">
            ...
            {openMenuId === invoice.id && (
              <div className="border-border/60 bg-popover animate-fade-in absolute right-0 z-20 mt-1 w-44 rounded-xl border p-1.5 shadow-xl">
                <Link
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    alert(`Invoice ${invoice.orderId} sent!`);
                    setOpenMenuId(null);
                  }}
                  className="text-foreground hover:bg-muted/60 flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors"
                >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-xl font-bold tracking-tight">Invoice</h1>
      ...
      {dialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <div className="border-border/60 bg-card relative mx-4 w-full max-w-md rounded-2xl border p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-foreground text-lg font-semibold">Delete Invoice</h2>
              </div>
              <button
            ...
            <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
              <p className="text-muted-foreground text-sm">
                ...
                <span className="text-foreground font-semibold">
                  {dialog.invoice.invoiceNo || dialog.invoice.orderId}
                </span>
