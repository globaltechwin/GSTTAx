...
export function CompaniesClient() {
  ...
  }, [page, pageSize, sorting, globalFilter, statusFilter, gstFilter]);
  ...
  useEffect(() => {
    setPage(1);
  }, [globalFilter, statusFilter, gstFilter, pageSize]);
  ...
  const handleAdd = async (formData: CompanyFormData) => {
  ...
  const handleEdit = async (formData: CompanyFormData) => {
    if (dialog?.type !== "edit") return;
    setActionLoading(true);
    try {
      const res = await fetch(`/api/companies/${dialog.company.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
  ...
  const columns: ColumnDef<Company>[] = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {info.row.original.address && (
              <p className="text-muted-foreground truncate text-xs">{info.row.original.address}</p>
            )}
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        const config = statusConfig[val] || statusConfig.active;
        return (
          <span
            className={cn(
            ...
            {openMenuId === company.id && (
              <div className="border-border/60 bg-popover animate-fade-in absolute right-0 z-20 mt-1 w-48 rounded-xl border p-1.5 shadow-xl">
                ...
                <button
                  ...
                  className="text-foreground hover:bg-muted/60 flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors disabled:opacity-50"
                >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-xl font-bold tracking-tight">Company</h1>
                  ...
                  <td colSpan={columns.length} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <div className="bg-muted/50 flex size-14 items-center justify-center rounded-2xl">
                        <Building2 className="text-muted-foreground/40 size-7" />
                      </div>
                      <div>
      ...
      {dialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="border-border/60 bg-card relative mx-4 w-full max-w-2xl rounded-2xl border p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-foreground text-lg font-semibold">
                  {dialog.type === "add" && "Add New Company"}
                  {dialog.type === "edit" && "Edit Company"}
                  {dialog.type === "view" && "Company Details"}
                  {dialog.type === "delete" && "Delete Company"}
                </h2>
              </div>
              {!actionLoading && (
                <button
            ...
            {dialog.type === "view" && (
              <CompanyView company={dialog.company} onClose={() => setDialog(null)} />
            )}
            {dialog.type === "delete" && (
